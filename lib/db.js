...
// alphanumérique, au moins un chiffre, et si des lettres sont présentes il en faut au moins deux.
// Appliquée aussi ici pour ne pas dépendre uniquement de la validation client (un autre appelant
// de cette même route pourrait l'omettre).
...
// Retire (met à vide) tout numéro dernier kilométrique déjà en base qui ne serait pas alphanumérique
// avec au moins 2 chiffres ET 2 lettres, ou qui correspondrait à un mot parasite connu — plus strict
// que la validation à l'écriture (isValidNumDernierKm, qui accepte par exemple une valeur 100%
// numérique). Ne supprime pas les lignes elles-mêmes, seulement la valeur invalide de ce champ.
  ...
  const { rows } = await sql`
    ...
    WHERE num_dernier_km IS NOT NULL AND num_dernier_km <> ''
      AND (
        ...
        OR length(regexp_replace(num_dernier_km, '[^0-9]', '', 'g')) < 2
        OR length(regexp_replace(num_dernier_km, '[^A-Za-z]', '', 'g')) < 2
