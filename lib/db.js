...
// partagée 'dashboard-excluded-carriers', lib/config.js) retire des transporteurs qui n'ont jamais
// de numéro dernier kilométrique par nature — sans quoi ils faussent à la baisse le taux de
// résolution, aussi bien par transporteur que dans `overall` (recalculé ici à partir des mêmes
// lignes filtrées, plutôt que via getStats() qui reste non filtré — utilisé ailleurs tel quel pour
// le compteur global de la page Base de données).
async function resolutionStats(excludedCarriers = []) {
  const [{ rows: entries }, { rows: overallRows }] = await Promise.all([
    sql`
      SELECT coalesce(nullif(transporteur, ''), '(sans transporteur)') AS transporteur,
             count(*)::int AS total,
             count(*) FILTER (WHERE num_dernier_km IS NOT NULL AND num_dernier_km <> '')::int AS resolved
      FROM colis
      WHERE NOT (transporteur = ANY(${excludedCarriers}::text[]))
      GROUP BY transporteur
      HAVING count(*) >= 100
      ORDER BY total DESC
    `,
  ...
  return { entries, overall: overallRows[0] || { total: 0, resolved: 0 } };
}
