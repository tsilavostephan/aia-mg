...
async function search(q, limit, offset) {
  ...
  const clauses = groups.map(terms => {
    ...
    return `search_text ILIKE ALL($${params.length}::text[])`;
  });
...
// regroupement CARRIERS/mapping manuel côté client) sur une fenêtre récente, groupés par
// jour/semaine/mois. Deux compteurs indépendants par (transporteur, créneau) plutôt qu'une seule
// requête : un colis résolu dans ce créneau n'a pas forcément été créé dans le même (scraping en
// retard sur l'import), donc "ajoutés" (created_at) et "résolus" (resolved_at) sont deux séries
// distinctes fusionnées ici plutôt qu'un ratio de cohorte strict.
async function resolutionStats(period) {
  const bucket = ['day', 'week', 'month'].includes(period) ? period : 'day';
  const lookbackDays = { day: 60, week: 182, month: 730 }[bucket];
  const since = new Date(Date.now() - lookbackDays * 86400000);

  const [{ rows: createdRows }, { rows: resolvedRows }] = await Promise.all([
    sql.query(
      `SELECT coalesce(nullif(transporteur, ''), '(sans transporteur)') AS transporteur,
       ...
       FROM colis
       WHERE created_at >= $2
       GROUP BY transporteur, bucket`,
      [bucket, since]
    ),
    sql.query(
      `SELECT coalesce(nullif(transporteur, ''), '(sans transporteur)') AS transporteur,
              date_trunc($1, resolved_at) AS bucket,
              count(*)::int AS resolved
       FROM colis
       WHERE resolved_at IS NOT NULL AND resolved_at >= $2
       GROUP BY transporteur, bucket`,
      [bucket, since]
    ),
  ]);
  ...
  for (const r of createdRows) {
    map.set(keyOf(r.transporteur, r.bucket.toISOString()), {
      transporteur: r.transporteur, bucket: r.bucket.toISOString(), total: r.total, resolved: 0,
    });
  }
  for (const r of resolvedRows) {
    const k = keyOf(r.transporteur, r.bucket.toISOString());
    const existing = map.get(k);
    if (existing) existing.resolved = r.resolved;
    else map.set(k, { transporteur: r.transporteur, bucket: r.bucket.toISOString(), total: 0, resolved: r.resolved });
  }

  const entries = Array.from(map.values())
    .sort((a, b) => (a.bucket < b.bucket ? 1 : a.bucket > b.bucket ? -1 : a.transporteur.localeCompare(b.transporteur)));
  return { period: bucket, entries };
}
