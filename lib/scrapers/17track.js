...
module.exports = async function handler(req, res) {
  ...
  // Pas d'encodeURIComponent ici : le lien "Ouvrir" du front (buildCarrierTrackingUrl() dans
  // assets/script.js) construit ce même lien avec des virgules littérales, non encodées (17TRACK
  // n'a pas urlEncodeNums:true) — c'est le format confirmé fonctionnel par test utilisateur réel.
  // %2C à la place de la virgule fait échouer le découpage des numéros côté 17TRACK, qui retombe
  // alors sur son colis de démonstration (TestNumber00017) au lieu des numéros demandés.
  ...
  try {
    ...
    // Une bannière d'info (ex. limitation USPS) peut s'afficher au-dessus du contenu et empêcher le
    // rendu/l'interaction normale avec la liste de résultats — confirmé par test utilisateur réel.
    // On la ferme dès que possible, avant même d'attendre les résultats.
    const bannerDebug = { closeBtnFound: false };
    try {
      const closeBtn = await page.$('#close-banner');
      bannerDebug.closeBtnFound = !!closeBtn;
      if (closeBtn) {
        await closeBtn.click().catch(() => {});
        await new Promise((r) => setTimeout(r, 500));
      }
    } catch (e) {
      bannerDebug.error = e && e.message;
    }
    ...
    const debug = { bannerDebug, resultsFound, expandDebug, requestedCount: trackingNumbers.length, resultCount: results.length };

    if (results.length === 0) {
      const domDebug = await page.evaluate(() => ({
        pageTitle: document.title,
        bodyTextPreview: (document.body ? document.body.innerText : '').slice(0, 1000),
        // Zone qui liste les résultats (voir sélecteur des cartes plus haut) : donne le HTML brut
        // exact reçu par le navigateur headless, pour diagnostiquer sans deviner.
        resultsAreaHtml: (document.querySelector('.flex-1.overflow-y-auto.custom-scrollbar') || document.body).outerHTML.slice(0, 20000),
      }));
      Object.assign(debug, domDebug);
      debug.structureChangeWarning = buildStructureChangeWarning(domDebug);
      // Capture visuelle exacte de ce que le navigateur headless affiche à ce moment précis.
      debug.screenshotBase64 = await page.screenshot({ encoding: 'base64' }).catch(() => null);
    }
