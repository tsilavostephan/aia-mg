...
// d'export groupé sur ce site. On filtre d'abord sur le statut "En transit" (onglet .next-box dont
// le .badge-title commence par "En transit") — seuls ces colis ont besoin d'être vérifiés, les
// autres statuts (livré, etc.) sont déjà résolus. On clique ensuite sur chaque élément de la liste
// filtrée un par un, on attend que le "Tracking No." affiché change par rapport au précédent (signe
// que le panneau de détail s'est mis à jour), puis on le lit.
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Filtre "En transit" (demande explicite de l'utilisateur) : seuls les colis pas encore livrés
    // ont besoin d'être vérifiés. L'onglet est repéré par son .badge-title (le texte inclut un
    // compteur entre parenthèses, ex. "En transit (7)", donc on ne teste que le début du texte).
    const filterHandle = await page.evaluateHandle(() => {
      const badge = Array.from(document.querySelectorAll('.next-box .badge-title'))
        .find((el) => /^en transit/i.test((el.textContent || '').trim()));
      return badge ? badge.closest('.next-box') : null;
    });
    const filterEl = filterHandle.asElement();
    const filterClicked = !!filterEl;
    if (filterEl) {
    ...
    const debug = { orderNumsFound, filterClicked, retryCount, clickedCount, unchangedCount, resultCount: results.length, stoppedEarly };
