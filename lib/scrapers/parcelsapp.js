...
// Bloque le chargement des images (inutiles pour lire le tableau parcel-attributes) pour accélérer
// chaque page — gain notable ici puisqu'on attend maintenant le chargement complet ('load').
...
module.exports = async function handler(req, res) {
  ...
  // Marge élargie à 15s (au lieu de 8s) : le chargement des modules (puppeteer-extra + évasions
  // stealth) au démarrage à froid se produit avant handler() et n'est donc pas compté dans
  // startTime, ce qui rognait la marge réelle avant la coupure à 60s (voir maxDuration, vercel.json).
  ...
  // Réduit à 2 (au lieu de 4) : PARCELSAPP et ORDERTRACKER (lib/scrapers/ordertracker.js, même
  // principe) peuvent tourner en même temps côté client, chacun lançant potentiellement plusieurs
  // invocations Vercel simultanées — un pool trop large par fonction multiplie le risque de saturer
  // Chromium (mémoire/CPU) une fois cumulé entre les deux, au prix d'un débit par lot un peu moindre.
  const PAGE_POOL_SIZE = 2;
  try {
    // Le lancement du navigateur (extraction du binaire Chromium au démarrage à froid) n'était pas
    // borné par DEADLINE_MS : s'il traîne (ou reste bloqué), la fonction se fait tuer par Vercel à
    // 60s (FUNCTION_INVOCATION_TIMEOUT, sans réponse) plutôt que de renvoyer une erreur exploitable.
    // On le fait donc courir contre l'échéance restante, comme le reste du traitement.
    ...
    async function processOne(page, num) {
      ...
      try {
        ...
        // plutôt que de démarrer dès que le DOM initial est prêt.
