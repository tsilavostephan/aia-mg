...
// Chaque appel est donc borné individuellement.
const FRAME_EVALUATE_TIMEOUT_MS = 2000;
...
async function scrapeOne(browser, num, deadline, pageLoadWaitMs, log = () => {}) {
  ...
  try {
    ...
    // (confirmé par capture d'écran) — plusieurs essais le temps qu'elle apparaisse réellement.
    let consentDismissed = false;
    for (let i = 0; i < 6 && !consentDismissed; i++) {
      consentDismissed = await dismissCookieConsent(page);
      if (!consentDismissed) await new Promise((r) => setTimeout(r, 500));
    }
