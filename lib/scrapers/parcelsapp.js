...
// Réduit à 2 (la demande initiale portait sur 5) : l'instrumentation temporaire a montré qu'avec 5
// onglets ouverts en même temps, chaque page.goto() prenait 20-30s+ au lieu de quelques secondes
// (100% d'échecs constatés en prod) — signe d'une famine CPU, les 5 pages se partageant le seul
// vCPU alloué à la fonction serverless Vercel. Compromis accepté pour obtenir des résultats.
...
async function scrapeOne(browser, num, deadline, pageLoadWaitMs, log = () => {}) {
  ...
  try {
    ...
    log(`  [${num}] goto terminé (status=${response ? response.status() : null}), waitForSelector...`);
