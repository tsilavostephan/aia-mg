...
async function scrapeOne(browser, num, deadline, pageLoadWaitMs, log = () => {}) {
  ...
  try {
    ...
    // PARCELSAPP_KEEP_COOKIES saute la suppression des cookies : utilisé par
    // scripts/local-scrape-worker.js quand il pointe directement sur le VRAI profil Chrome de
    // l'utilisateur (pas un profil jetable) — sinon ce nettoyage supprimerait le cookie de
    // consentement nécessaire dès le premier colis traité, cassant tous les suivants.
    if (process.env.PARCELSAPP_KEEP_COOKIES) {
      log(`  [${num}] nettoyage (fermeture seule, cookies conservés)...`);
    } else {
