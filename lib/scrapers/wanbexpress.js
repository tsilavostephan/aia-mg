...
//
// Contre l'anti-bot et les iframes publicitaires tierces potentiellement bloquantes : plugin
// stealth, aucun blocage de requêtes/images (risque de détection comme ad-blocker sur ce genre de
// site, prudence appliquée par défaut), fermeture de bannière de consentement bornée dans le temps
// total (pas juste par frame), et tous les appels de nettoyage (cookies, page.close, browser.close)
...
// Truc pour le traceur de fichiers de Vercel (@vercel/nft) : puppeteer-extra résout les dépendances
// de ses plugins (stealth → user-preferences → user-data-dir) via un require() calculé à
...
async function scrapeOne(browser, num, deadline, pageLoadWaitMs, log = () => {}) {
  ...
  try {
    ...
    // Pas de blocage des images/requêtes ici : sur ce genre de site, ça peut se faire détecter
    // comme un bloqueur de pub et casser le rendu — prudence appliquée par défaut.
...
// pour scripts/local-scrape-worker.js (voir ce fichier), qui lance son propre navigateur local.
