...
// partagé par de vrais comptes avec rôle (pending/mobile/pc/admin).
...
async function findUserByEmail(email) {
  ...
  const { rows } = await sql`SELECT * FROM users WHERE lower(email) = ${cleaned} LIMIT 1`;
...
// role optionnel : utilisé uniquement pour l'amorçage du premier admin (APP_BOOTSTRAP_ADMIN_EMAIL),
// sinon 'pending' par défaut (colonne DEFAULT).
async function createUser(email, password, role) {
  const cleaned = String(email || '').trim().toLowerCase();
  const passwordHash = hashPassword(password);
  const { rows } = role
    ? await sql`INSERT INTO users (email, password_hash, role, approved_at) VALUES (${cleaned}, ${passwordHash}, ${role}, now()) RETURNING *`
    : await sql`INSERT INTO users (email, password_hash) VALUES (${cleaned}, ${passwordHash}) RETURNING *`;
  return rows[0];
}

async function listUsers() {
  const { rows } = await sql`SELECT id, email, role, created_at, approved_at FROM users ORDER BY created_at DESC`;
...
async function setUserRole(id, role) {
  ...
  const { rows } = await sql`
    ...
    WHERE id = ${id}
    RETURNING id, email, role, created_at, approved_at
  `;
